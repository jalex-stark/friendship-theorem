...
import sym_matrix findicator eigen
...
def adjacency_matrix {V:Type*} [fintype V] (G: fin_graph V): matrix V V ℕ :=
    λ v w:V, prop_to_nat(G.E v w)
...
theorem adj_mat_traceless {V:Type*} [fintype V] (G: fin_graph V):
    matrix.trace V ℝ ℝ (adjacency_matrix G: matrix V V ℤ)= 0 := by simp
...
begin
    ...
    have hfind:(λ (i : V), 1) = findicator finset.univ,
    ext,
    symmetry,
    rw ind_one_iff_mem,
    apply finset.mem_univ,
